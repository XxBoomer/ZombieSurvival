...
class ZScommands
{
    ...
    public function __construct(ZSmain $plugin)
    {
        $this->pg = $plugin;
    }
    ...
    public function onCommand(CommandSender $sender, Command $command, $label, array $args)
    {
        if (!($sender instanceof Player) || !$sender->isOp()) {
            switch (strtolower(array_shift($args))):
                ...
                case 'join':
                    if (!(count($args) < 0b11)) {
                        $sender->sendMessage(TextFormat::AQUA . '→' . TextFormat::RED . 'Usage: /zs ' . TextFormat::GREEN . 'join [ZSname]' . TextFormat::GRAY . ' [PlayerName]');
                        break;
                    }
                    ...
                    if (isset($args[0])) {
                        //ZS NAME
                        $ZSname = TextFormat::clean(array_shift($args));
                        if (!array_key_exists($SWname, $this->pg->arenas)) {
                            $sender->sendMessage(TextFormat::AQUA . '→' . TextFormat::RED . 'Arena with name: ' . TextFormat::WHITE . $ZSname . TextFormat::RED . ' doesn\'t exist');
                            break;
                        }
                    } else {
                    ...
                    if (strlen($player) > 0 && $sender instanceof \pocketmine\command\ConsoleCommandSender) {
                        ...
                        $this->pg->arenas[$ZSname]->join($sender);
                    } else {
                        $sender->sendMessage(TextFormat::RED . 'Player not found!');
                    }
                    break;
                ...
                case 'quit':
                    if (!empty($args)) {
                        $sender->sendMessage(TextFormat::AQUA . '→' . TextFormat::RED . 'Usage: /zs ' . TextFormat::GREEN . 'quit');
                        break;
                    }
                ...
                default:
                    ...
                    $sender->sendMessage(TextFormat::AQUA . '→' . TextFormat::RED . 'Usage: /zs [join|quit]');
                    break;
        ...
        switch (strtolower(array_shift($args))):
            ...
            case 'create':
                ...
                */
                if (!(count($args) > 0b11 && count($args) < 0b101)) {
                    $sender->sendMessage(TextFormat::AQUA . '→' . TextFormat::RED . 'Usage: /zs ' . TextFormat::GREEN . 'create [ZSname] [slots] [countdown] [maxGameTime]');
                    break;
                }
                ...
                foreach ($this->pg->arenas as $aname => $arena) {
                    if ($arena->getWorld() == $world) {
                        ...
                        $sender->sendMessage(TextFormat::RED . '→' . TextFormat::WHITE . '/zs list' . TextFormat::RED . ' for a list of arenas');
                        $sender->sendMessage(TextFormat::RED . '→' . TextFormat::WHITE . '/zs delete' . TextFormat::RED . ' to delete an arena');
                        unset($fworld, $world);
                        break 2;
                    }
                }
                ...
                foreach ($this->pg->signs as $loc => $name) {
                    if (explode(':', $loc)[3] == $world) {
                        ...
                        $sender->sendMessage(TextFormat::RED . '→' . TextFormat::WHITE . '/zs signdelete' . TextFormat::RED . ' to delete signs');
                        unset($fworld, $world);
                        break 2;
                    }
                }
                ...
                //ZS NAME
                $ZSname = array_shift($args);
                if (!($ZSname && preg_match('/^[a-z0-9]+[a-z0-9]$/i', $ZSname) && strlen($ZSname) < 0x10 && strlen($ZSname) > 0b10)) {
                    $sender->sendMessage(TextFormat::WHITE . '→' . TextFormat::AQUA . '[ZSname]' . TextFormat::RED . ' must consists of a-z 0-9 (min3-max15)');
                    unset($fworld, $world, $ZSname);
                    break;
                }


                //Checks if the arena already exists
                if (array_key_exists($ZSname, $this->pg->arenas)) {
                    $sender->sendMessage(TextFormat::AQUA . '→' . TextFormat::RED . 'Arena with name: ' . TextFormat::WHITE . $ZSname . TextFormat::RED . ' already exist');
                    unset($fworld, $world, $ZSname);
                    break;
                }
                ...
                $tar = new \PharData($this->pg->getDataFolder() . 'arenas/' . $ZSname . '/' . $world . '.tar');
                $tar->startBuffering();
                ...
                //ZSarena object
                $this->pg->arenas[$ZSname] = new ZSarena($this->pg, $ZSname, $slot, $world, $countdown, $maxtime, $void);
                $sender->sendMessage(TextFormat::AQUA . '→' . TextFormat::GREEN . 'Arena: ' . TextFormat::DARK_GREEN . $ZSname . TextFormat::GREEN . ' created successfully!');
                $sender->sendMessage(TextFormat::AQUA . '→' . TextFormat::GREEN . 'Now set spawns with ' . TextFormat::WHITE . '/zs setspawn [slot]');
                $sender->teleport($sender->getServer()->getLevelByName($world)->getSpawnLocation());
                unset($fworld, $world, $ZSname, $slot, $countdown, $maxtime, $provider, $void);
                break;
            ...
            case 'setspawn':
                ...
                */
                if (count($args) != 1) {
                    $sender->sendMessage(TextFormat::AQUA . '→' . TextFormat::RED . 'Usage: /zs ' . TextFormat::GREEN . 'setspawn [slot]');
                    break;
                }
                ...
                if (!($ZSname && preg_match('/^[a-z0-9]+[a-z0-9]$/i', $ZSname) && strlen($ZSname) < 0x10 && strlen($ZSname) > 0b10 && array_key_exists($ZSname, $this->pg->arenas))) {
                    $sender->sendMessage(TextFormat::AQUA . '→' . TextFormat::RED . 'Arena not found here, try ' . TextFormat::WHITE . '/zs create');
                    unset($ZSname);
                    break;
                }
                ...
                $slot += 0;
                if ($sender->getLevel()->getName() == $this->pg->arenas[$ZSname]->getWorld()) {
                    if ($this->pg->arenas[$ZSname]->setSpawn($sender, $slot)) {
                        $sender->sendMessage(TextFormat::AQUA . '→' . TextFormat::GREEN . 'New spawn: ' . TextFormat::WHITE . $slot . TextFormat::GREEN . ' In arena: ' . TextFormat::WHITE . $ZSname);
                        if ($this->pg->arenas[$SWname]->checkSpawns())
                            $sender->sendMessage(TextFormat::AQUA . '→' . TextFormat::GREEN . 'I found all the spawns for Arena: ' . TextFormat::WHITE . $ZSname . TextFormat::GREEN . ', now you can create a join sign!');
                    }
                }
                break;
            ...
            case 'delete':
                ...
                */
                if (count($args) != 1) {
                    $sender->sendMessage(TextFormat::AQUA . '→' . TextFormat::RED . 'Usage: /zs ' . TextFormat::GREEN . 'delete [ZSname]');
                    break;
                }
                ...
                $ZSname = array_shift($args);
                if (!($ZSname && preg_match('/^[a-z0-9]+[a-z0-9]$/i', $ZSname) && strlen($ZSname) < 0x10 && strlen($ZSname) > 0b10 && array_key_exists($ZSname, $this->pg->arenas))) {
                    $sender->sendMessage(TextFormat::AQUA . '→' . TextFormat::RED . 'Arena: ' . TextFormat::WHITE . $ZSname . TextFormat::RED . ' doesn\'t exist');
                    unset($ZSname);
                    break;
                }


                if (!(is_dir($this->pg->getDataFolder() . 'arenas/' . $ZSname) && is_file($this->pg->getDataFolder() . 'arenas/' . $ZSname . '/settings.yml'))) {
                ...
                $this->pg->arenas[$ZSname]->stop(true);
                foreach ($this->pg->signs as $loc => $name) {
                    if ($ZSname == $name) {
                        $ex = explode(':', $loc);
                ...
                unset($this->pg->arenas[$ZSname]);


                foreach (scandir($this->pg->getDataFolder() . 'arenas/' . $ZSname) as $file) {
                    if ($file != '.' && $file != '..' && is_file($this->pg->getDataFolder() . 'arenas/' . $ZSname . '/' . $file)) {
                        @unlink($this->pg->getDataFolder() . 'arenas/' . $ZSname . '/' . $file);
                    }
                }
                @rmdir($this->pg->getDataFolder() . 'arenas/' . $ZSname);
            ...
            case 'signdelete':
                ...
                */
                if (count($args) != 1) {
                    $sender->sendMessage(TextFormat::AQUA . '→' . TextFormat::RED . 'Usage: /zs ' . TextFormat::GREEN . 'signdelete [ZSname|all]');
                    break;
                }
                ...
                if (!array_key_exists($ZSname, $this->pg->arenas)) {
                    if ($ZSname == 'all') {
                        ...
                        $this->pg->setSign($ZSname, 0, 0, 0, 'world', true);
                        $sender->sendMessage(TextFormat::AQUA . '→' . TextFormat::GREEN . 'Deleted all ZS signs !');
                        unset($ZSname, $loc, $name, $ex, $block);
                    } else {
                        $sender->sendMessage(TextFormat::AQUA . '→' . TextFormat::RED . 'Arena: ' . TextFormat::WHITE . $ZSname . TextFormat::RED . ' doesn\'t exist');
                        unset($ZSname);
                    }
